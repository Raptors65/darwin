...
from webhooks import verify_signature, handle_pr_event, handle_review_event
...
async def fix_task(task_id: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            similar_fixes = await get_similar_successful_fixes(task_data, redis_client)
            similar_fixes_text = format_similar_fixes(similar_fixes)
            if similar_fixes:
                logger.info("Found %d similar successful fixes to learn from", len(similar_fixes))
            ...
            style_rules_text = ""
            if product:
                style_rules = await get_top_rules_for_product(redis_client, product, limit=10)
                style_rules_text = format_rules_for_prompt(style_rules)
                if style_rules:
                    logger.info("Found %d style rules for product %s", len(style_rules), product)
                    # Increment usage counters for each rule
                    for rule in style_rules:
                        await increment_rule_usage(redis_client, product, rule.get("id", ""))

            # Run the fix agent with similar fixes and style rules context
...
# =============================================================================
# GitHub Webhooks (for self-improvement feedback loop)
# =============================================================================
...
class CreateRuleRequest(BaseModel):
    """Request to create a new style rule."""
    content: str
    category: str


@app.get("/products/{product}/rules")
async def list_product_rules(product: str):
    """List all style rules for a product.
    
    Args:
        product: The product name (e.g., "joplin").
        
    Returns:
        List of all rules for the product.
    """
    try:
        redis_client = await get_redis()
        rules = await list_all_rules_for_product(redis_client, product)
        return {"product": product, "rules": rules, "count": len(rules)}
    except Exception as e:
        logger.exception("Failed to list rules for product %s: %s", product, e)
        raise HTTPException(status_code=500, detail=str(e)) from e


@app.post("/products/{product}/rules")
async def add_product_rule(product: str, request: CreateRuleRequest):
    """Add a new manual style rule for a product.
    
    Args:
        product: The product name.
        request: The rule content and category.
    ...
    try:
        if request.category not in RULE_CATEGORIES:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid category: {request.category}. Must be one of {RULE_CATEGORIES}",
            )
        ...
        rule = await get_rule(redis_client, product, rule_id)
        return rule
        
    except HTTPException:
        raise
    except Exception as e:
        logger.exception("Failed to create rule for product %s: %s", product, e)
        raise HTTPException(status_code=500, detail=str(e)) from e


@app.delete("/products/{product}/rules/{rule_id}")
async def remove_product_rule(product: str, rule_id: str):
    """Delete a style rule.
    ...
    try:
        redis_client = await get_redis()
        deleted = await delete_rule(redis_client, product, rule_id)
        
        if not deleted:
            raise HTTPException(status_code=404, detail="Rule not found")
        
        return {"status": "deleted", "rule_id": rule_id, "product": product}
        
    except HTTPException:
        raise
    except Exception as e:
        logger.exception("Failed to delete rule %s for product %s: %s", rule_id, product, e)
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def github_webhook(request: Request):
    """Handle GitHub webhook events for PR feedback.
    
    This endpoint receives webhooks from GitHub when PRs are:
    - Merged: Store as successful fix for future learning
    - Closed: Mark as rejected
    - Reviewed: Capture feedback (future enhancement)
    
    The webhook secret is verified to ensure requests come from GitHub.
    """
    ...
    try:
        if event_type == "pull_request":
            ...
            result = await handle_pr_event(action, pr_data, redis_client)
            return result
            ...
            pr_data = payload.get("pull_request", {})
            result = await handle_review_event(action, review_data, pr_data, redis_client)
            return result
        ...
        logger.exception("Error handling GitHub webhook: %s", e)
        raise HTTPException(status_code=500, detail=str(e)) from e
